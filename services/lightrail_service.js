...
function readTrainTimesFromCSV(next_times, stop_number, file_string, cb){
  fs.readFile(file_string, function(err, file){
    ...
    csv.parse(file,null,function(err,train_schedule_csv){
      if(err){
        console.log(err);
        cb(null);
        return;
      ...
      while(go)
      {
        train_time = train_schedule_csv[i][stop_number];
        if(train_time == '-----') {
          i++;
          continue;
        }
      ...
      {
        if(typeof train_schedule_csv[i+j] == "undefined")
        {
          outArray.push('End of Service');
          cb(outArray);
          return;
        }
        if(train_schedule_csv[i + j][stop_number].indexOf('-----') != -1)
        {
          i++;
          j--;
          continue;
        }
        outArray.push(train_schedule_csv[i + j][stop_number]);
      }
      cb(outArray);
    });
  });
